...
// Reemplaza el contenido temporal anterior de 97_ACTUALIZACION_QA.gs.
// No contiene preguntas ni vocabulario. No usar en producción.

var ELIVE176_VERSION = 'CS21A176';
var ELIVE176_UPDATE_OBJECTIVE = 'Turnos compartidos, tablero sincronizado y acceso estudiante Memory Match';
...
function _elive176PairRows_(level, unit) {
  ...
  return _elive176Rows_('QUESTION_BANK').filter(function (row) {
    ...
    return _elive176Upper_(row.GAME_ID) === ELMM174_GAME_CODE &&
      ...
      _elive176Text_(row.PAIR_LEFT || row.STEM) &&
      _elive176Text_(row.PAIR_RIGHT);
  });
}
...
function _elive176Cards_(room, pairCount) {
  ...
  rows.forEach(function (row, index) {
    ...
    cards.push({
      ...
      label:_elive176Text_(row.PAIR_LEFT || row.STEM), media_id:_elive176Text_(row.MEDIA_ID)
    });
    cards.push({
      card_id:pairId + '-R', pair_id:pairId, face_type:'TEXT',
      label:_elive176Text_(row.PAIR_RIGHT), media_id:''
    });
  });
...
function _elive176PlayerRows_(room) {
  ...
  return values.slice(1).filter(function (row) {
    return _elive176Text_(row[index.ROOM_CODE]) === _elive176Text_(room.ROOM_CODE);
  }).map(function (row) {
    var playerId = _elive176Text_(row[index.COD_ESTUDIANTE] || row[index.PLAYER_ID]);
    return {
      player_id:playerId,
      name:_elive176Text_(row[index.NOMBRE]) || playerId,
      team_id:_elive176Text_(row[index.TEAM]) || 'NO_TEAM',
      joined_at:_elive176Text_(row[index.JOINED_AT] || row[index.CREATED_AT])
    };
...
function _elive176GroupTeams_(players) {
  ...
  return Object.keys(grouped).map(function (teamId) {
    return {team_id:teamId,name:teamId === 'NO_TEAM' ? 'Sin equipo' : teamId,members:grouped[teamId]};
  });
}
...
function _elive176CreateTurnState_(room, players, policy, startAt, durationMs) {
  ...
  var state = {
    ...
    active_team_id:'',
    turn_number:1,
    turn_started_at:_elive176Iso_(startAt),
    turn_ends_at:_elive176Iso_(new Date(startAt.getTime() + Math.max(5000, Number(durationMs || 30000) || 30000))),
    last_player_id:'',
    last_team_id:'',
  ...
  if (policy === ELIVE176_POLICY_TEAM_ALTERNATING) {
    ...
    teams.forEach(function (team) {
      state.team_player_orders[team.team_id] = _elive176Shuffle_(
        team.members.map(function (member) { return member.player_id; }),
        seed + '|TEAM|' + team.team_id
      );
...
function _elive176NextTurn_(turnState, now, durationMs, reason) {
  ...
  if (current.participation_policy === ELIVE176_POLICY_TEAM_ALTERNATING) {
    ...
    if (previousMembers.length) {
      next.team_player_cursors[previousTeam] = (Number(next.team_player_cursors[previousTeam] || 0) + 1) % previousMembers.length;
    }
...
function _elive176CanAct_(turnState, player) {
  ...
  if (turnState.participation_policy === ELIVE176_POLICY_TEAM_ALTERNATING) {
    return !_elive176Text_(turnState.active_team_id) ||
      _elive176Text_(turnState.active_team_id) === _elive176Text_(player.team_id);
  }
  return true;
}
...
function _elive176DescribeTurn_(turnState, players) {
  ...
  return {
    active_player:byId[turnState.active_player_id] || null,
    next_player:byId[nextPlayerId] || null,
    active_team_id:_elive176Text_(turnState.active_team_id),
    turn_number:Number(turnState.turn_number || 1) || 1,
    participation_policy:_elive176Text_(turnState.participation_policy)
  };
}
...
function englishLabMemoryMatchCreateRoomCS21A176(body) {
  ...
  if (result && result.ok === true) {
    ...
    result.update_objective = ELIVE176_UPDATE_OBJECTIVE;
  }
  return result;
}
...
function englishLabMemoryMatchStartRoomCS21A176(body) {
  ...
  var policy = _elive176Upper_(room.MODE) === 'TEAMS'
    ? ELIVE176_POLICY_TEAM_ALTERNATING
    : ELIVE176_POLICY_RANDOM_PLAYER;
  ...
  var row = _eliveSetCells_(managed.found, {
    STATUS:'LIVE', STARTED_AT:room.STARTED_AT || _elive176Iso_(now), CURRENT_INDEX:1,
    ROUND_STATUS:'OPEN', ROUND_STARTED_AT:_elive176Iso_(now), ROUND_CLOSED_AT:'',
  ...
  return {
    ok:true,
    version:ELIVE176_VERSION,
    room:_elive176PublicRoom_(row),
    room_package:pkg,
    turn_description:_elive176DescribeTurn_(turnState, players)
  };
}
...
function englishLabMemoryMatchGetPlayerStateCS21A176(body) {
  ...
  base.can_answer = !!(pkg && pkg.state && pkg.state.phase === 'OPEN' && _elive176CanAct_(turnState, currentPlayer));
  return base;
}
...
function _elive176PairFromBody_(pkg, body) {
  ...
  return {
    ...
    second_id:secondId,
    correct:correct,
    pair_id:correct ? _elive176Text_(first.pair_id) : ''
  };
}
...
function englishLabMemoryMatchSubmitPairCS21A176(body) {
  ...
  try {
    ...
    var players = _elive176PlayerRows_(room);
    var playerId = _elive176Text_(body.player_id || body.playerId || body.cod_estudiante || body.codigo_estudiante);
    var player = players.filter(function (item) { return item.player_id === playerId; })[0] || null;
    var turnState = pkg.turn_state || null;
    if (!_elive176CanAct_(turnState, player)) {
      return {
        ok:false,
        error:'turno_no_activo',
        mensaje:'Espere su turno.',
        turn_state:turnState,
        turn_description:turnState ? _elive176DescribeTurn_(turnState, players) : null
      };
    }
    ...
    var actionKey = [room.ROOM_CODE, turnState.turn_number, playerId, pair.first_id, pair.second_id].join('|');
    if (_elive176Text_(shared.last_action_key) === actionKey) {
      return {
        ok:true,
        accepted:false,
        duplicate:true,
        message:'Intento ya procesado.',
        room_package:pkg,
        turn_state:turnState,
        shared_state:shared
      };
    ...
    if (completed) {
      ...
      patch.ROUND_CLOSED_AT = _elive176Iso_(now);
    }
    var updatedRoom = _elive176SavePackage_(found, current, pkg, patch);
    _eliveAppendEvent_(updatedRoom, 'LIVE_TURN_ADVANCED', {sesion:{nombre:player.name},rol:'student'}, {
...
function verificarActualizacionQA() {
  ...
  var syntheticPlayers = [
    {player_id:'P1',name:'Jugador 1',team_id:'AZUL'},
    {player_id:'P2',name:'Jugador 2',team_id:'ROJO'},
    {player_id:'P3',name:'Jugador 3',team_id:'AZUL'},
    {player_id:'P4',name:'Jugador 4',team_id:'ROJO'}
  ];
  var syntheticRoom = {ROOM_CODE:'LAB-TEST',CURRENT_INDEX:1};
  var individual = _elive176CreateTurnState_(syntheticRoom, syntheticPlayers, ELIVE176_POLICY_RANDOM_PLAYER, new Date(), 30000);
  var teams = _elive176CreateTurnState_(syntheticRoom, syntheticPlayers, ELIVE176_POLICY_TEAM_ALTERNATING, new Date(), 30000);
  var teamsNext = _elive176NextTurn_(teams, new Date(), 30000, 'TEST');
  var result = {
    ok:pairsU1.length >= 6 && pairsU01.length >= 6 &&
      individual.player_order.length === 4 && !!individual.active_player_id &&
      teams.team_order.length === 2 && teams.active_team_id !== teamsNext.active_team_id,
    version:ELIVE176_VERSION,
    objective:ELIVE176_UPDATE_OBJECTIVE,
    normalize_u1:_elive176NormalizeUnit_('U1'),
    b1_u1_pairs:pairsU1.length,
    b1_u01_pairs:pairsU01.length,
    individual_active_player:individual.active_player_id,
    team_active:teams.active_team_id,
    team_next:teamsNext.active_team_id
  };
  console.log(JSON.stringify(result));
  if (!result.ok) throw new Error('CS21A176 no superó la verificación de banco y turnos.');
  return result;
}
...
doPost = function (e) {
  try {
    ...
    if (fn === 'englishlabmemorymatchcreateroom') {
      return _an4406_json_(englishLabMemoryMatchCreateRoomCS21A176(body));
    }
    if (fn === 'englishlabmemorymatchstartroom') {
      return _an4406_json_(englishLabMemoryMatchStartRoomCS21A176(body));
    }
    if (fn === 'englishlabmemorymatchgetplayerstate') {
      return _an4406_json_(englishLabMemoryMatchGetPlayerStateCS21A176(body));
    }
    if (fn === 'englishlablivegetplayerstate' && _elive176IsMemoryRoom_(body)) {
      return _an4406_json_(englishLabMemoryMatchGetPlayerStateCS21A176(body));
    }
    if (fn === 'englishlabmemorymatchgetroomcontrol') {
      return _an4406_json_(englishLabMemoryMatchGetRoomControlCS21A176(body));
    }
    if (fn === 'englishlabmemorymatchsubmitpair') {
      return _an4406_json_(englishLabMemoryMatchSubmitPairCS21A176(body));
    }
    if (fn === 'verificaractualizacionqa') {
      return _an4406_json_(verificarActualizacionQA());
    }
