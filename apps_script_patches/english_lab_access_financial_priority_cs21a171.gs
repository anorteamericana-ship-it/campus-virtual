...
// CS21A171: cuando el motor financiero publica mora_exigible o deuda_exigible,
// ese monto es la fuente canónica. Solo se usan mora_calculada/moroso/estado_financiero
// como fallback para expedientes históricos que todavía no exponen monto exigible.
try {
  _cs21a144DebtEvidence_ = function(row) {
    ...
    if (moraFlag === 'SI' || row.moroso === true || estado === 'DEUDA') {
      return { known:true, debt:true, source:'legacy_flag' };
    }
    if (moraFlag === 'NO' || row.moroso === false || estado === 'AL_DIA') {
      return { known:true, debt:false, source:'legacy_flag' };
    }
...
try {
  ...
  _cs21a144AccessBySession_ = function(body) {
    ...
    var access;
    try {
      // Lectura canónica con caché corto. Las escrituras críticas invalidan la ficha.
