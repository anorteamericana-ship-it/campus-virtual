  ...
  return page.evaluate(({ regionId }) => {
    ...
    try {
      ...
      for (const [key, region] of Object.entries(regions)) {
        const ajax = clean(region?.ajaxIdentifier || region?.ajax_identifier || '');
        if (ajax && valid(ajax)) return ajax.replace(/^PLUGIN=/i, '');
        const elementId = clean(region?.element?.attr?.('id') || region?.element?.[0]?.id || '');
        const regionKey = clean(region?.id || region?.regionId || key);
        if (ajax && (elementId.includes(regionId) || regionKey.includes(regionId))) {
          return ajax.replace(/^PLUGIN=/i, '');
        }
      }
    } catch {}
...
async function buildDirectDownloadLinkRequestUrl(page, ajaxIdentifier) {
  return page.evaluate(({ ajaxIdentifier, regionId, widgetId, reportId }) => {
    ...
    const generated = apexServer.pluginUrl(ajaxIdentifier, {
      x01:widgetId,
      x02:reportId,
      f01:[`R${regionId}_download_format`],
      f02:['CSV'],
    });
    if (!generated) return '';
...
async function downloadProspectCsvViaApexDirect(page, parseProspectCsv) {
  ...
  try {
    ...
    if (parsedRequestUrl.origin !== CONAPE_ORIGIN || !/\/wwv_flow\.ajax(?:$|\?)/.test(parsedRequestUrl.pathname + parsedRequestUrl.search)) {
...
async function downloadProspectCsvViaDialog(page, parseProspectCsv) {
  ...
  try {
    ...
    return {
      ok:false,
      reason:safeReason(error, 'CSV_V444_DOWNLOAD_FAILED'),
      columns_ok:false,
      rows:[],
    };
  }
