...
// F98.4-Z6-CS7B · Bandeja interna conectada con ventas/admisiones.
...
const F96_LAZY = {
  ...
  teacher_views: ['src/vista_docente.jsx?v=F98.4Z6CS19E','src/teacher_views.jsx?v=F98.4Z6CS19E','src/teacher_agenda_slots_cs19e.jsx?v=F98.4Z6CS19E'],
  vista_docente: ['src/vista_docente.jsx?v=F98.4Z6CS19E'],
  ...
  cronograma_grupo: ['src/vista_docente.jsx?v=F98.4Z6CS19E','src/cronograma_todos.jsx?v=F98.4Z6CM','src/cronograma_grupo.jsx?v=F98.4Z6CM'],
  calendario_grupo: ['src/vista_docente.jsx?v=F98.4Z6CS19E','src/cronograma_todos.jsx?v=F98.4Z6CM','src/cronograma_grupo.jsx?v=F98.4Z6CM','src/admin_students.jsx?v=F98.4Z6CD','src/calendario_grupo.jsx?v=F98.4Z6CM'],
  docente_operativo: ['src/vista_docente.jsx?v=F98.4Z6CS19E','src/teacher_views.jsx?v=F98.4Z6CS19E','src/teacher_agenda_slots_cs19e.jsx?v=F98.4Z6CS19E','src/docente_operativo.jsx?v=F96.5G'],
  ...
  supervision: ['src/vista_docente.jsx?v=F98.4Z6CS19E','src/panel_admin_supervision.jsx?v=F96.5G'],
  auditoria: ['src/auditoria_academica.jsx?v=F96.5G'],
  diagnostico: ['src/diagnostico_interno.jsx?v=F98.4Z6BD'],
  ...
  student_course: [
    'src/vista_docente.jsx?v=F98.4Z6CS19E','src/cronograma_todos.jsx?v=F98.4Z6CK','src/cronograma_grupo.jsx?v=F98.4Z6CE',
...
// F98.4-A · compatibilidad de rutas antiguas del estudiante.
const STUDENT_ROUTE_ALIASES_F984 = {
...
function normalizarRutaEstudianteF984(target, opts = {}) {
  const raw = String(target || 'dashboard').trim();
  const found = STUDENT_ROUTE_ALIASES_F984[raw] || ['dashboard', null];
  return { target: found[0], tab: opts.tab || opts.studentTab || found[1] || null };
}
...
const APP_WRITTEN_DEFS_F984N = [
  {key:'ESCRITO_1',leccion:18,titulo:'1.er Examen Escrito'},
  {key:'ESCRITO_2',leccion:32,titulo:'2.º Examen Escrito'},
];
function appEvaluationTypeF984N(e){
  const direct=String(e?.tipo_oficial||e?.tipo||'').toUpperCase();
  if([...APP_ORAL_DEFS_F927,...APP_WRITTEN_DEFS_F984N].some(d=>d.key===direct))return direct;
  const t=String(e?.titulo||'').toLowerCase();
  if(t.includes('1.er examen oral'))return'ORAL_1';
  if(t.includes('2.º examen oral')||t.includes('2.o examen oral'))return'ORAL_2';
  if(t.includes('3.er examen oral'))return'ORAL_3';
  if(t.includes('4.º examen oral')||t.includes('4.o examen oral'))return'ORAL_4';
  if(t.includes('1.er examen escrito'))return'ESCRITO_1';
  if(t.includes('2.º examen escrito')||t.includes('2.o examen escrito'))return'ESCRITO_2';
  return'';
}
function appLevelOrderF927(n){return({B1:1,B2:2,I1:3,I2:4})[String(n||'').toUpperCase()]||0;}
function appStudentEvaluationRowsF984N(defs,state,level,activeRepos){
  return defs.map(def=>{
    const ev=state.evals.find(e=>String(e.nivel||'').toUpperCase()===level&&appEvaluationTypeF984N(e)===def.key)||{};
...
function StudentEvaluationSectionF984N({kicker,title,rows,loading,error,onReload,loadingText}){
  return <div style={{border:'1px solid var(--line)',borderRadius:14,background:'#fff',overflow:'hidden'}}>
    <div style={{padding:'13px 15px',borderBottom:'1px solid var(--line)',background:'#F8FAFE',display:'flex',justifyContent:'space-between',gap:12,alignItems:'center',flexWrap:'wrap'}}><div><div style={{fontSize:10,fontWeight:900,letterSpacing:'.13em',color:'#7A1E2C'}}>{kicker}</div><div style={{fontSize:18,fontWeight:900,marginTop:3}}>{title}</div></div>{error&&<button className="btn btn-ghost" onClick={onReload}>Recargar</button>}</div>
    {loading?<div style={{padding:18,color:'var(--ink-3)'}}>{loadingText}</div>:error?<div style={{padding:18,color:'#991B1B'}}>{error}</div>:<div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(210px,1fr))',gap:10,padding:12}}>{rows.map(r=><div key={r.key} style={{border:'1px solid var(--line)',borderRadius:12,padding:13}}><div style={{fontSize:12.5,fontWeight:850}}>{r.titulo}</div><div style={{fontSize:10.5,color:'var(--ink-3)',marginTop:3}}>Lección {String(r.leccion).padStart(2,'0')}</div><span style={{display:'inline-flex',marginTop:9,padding:'5px 8px',borderRadius:999,background:r.bg,color:r.tone,fontSize:10,fontWeight:900}}>{r.label}</span></div>)}</div>}
  </div>;
}
function StudentEvaluationsOverviewF984N(){
  const ses=typeof getSesion==='function'?getSesion():{};
  ...
  const oralRows=appStudentEvaluationRowsF984N(APP_ORAL_DEFS_F927,state,level,activeRepos);
  const writtenRows=appStudentEvaluationRowsF984N(APP_WRITTEN_DEFS_F984N,state,level,activeRepos);
  return <>
    <StudentEvaluationSectionF984N kicker="EVALUACIONES ORALES" title={`Exámenes orales · ${level||'nivel actual'}`} rows={oralRows} loading={state.loading} error={state.error} onReload={load} loadingText="Consultando evaluaciones orales…" />
    <StudentEvaluationSectionF984N kicker="EVALUACIONES ESCRITAS" title={`Exámenes escritos · ${level||'nivel actual'}`} rows={writtenRows} loading={state.loading} error={state.error} onReload={load} loadingText="Consultando evaluaciones escritas…" />
  </>;
}
...
function ExamenesStudentPanel() {
  const top=<StudentEvaluationsOverviewF984N/>;
...
function App() {
  ...
  const [studentCourseTab, setStudentCourseTab] = useState('cronograma');
  const [studentEvalTab, setStudentEvalTab] = useState('proximas');
  const [studentDocsTab, setStudentDocsTab] = useState('programa');
  ...
  const navigateTo = (target, opts = {}) => {
    ...
    if (role === 'student') {
      ...
      if (finalTarget === 'mi_curso' && resolvedTab) setStudentCourseTab(resolvedTab);
      if (finalTarget === 'evaluaciones' && resolvedTab) setStudentEvalTab(resolvedTab);
      if (finalTarget === 'documentos_ayuda' && resolvedTab) setStudentDocsTab(resolvedTab);
    }
    ...
    } else if (finalTarget !== 'examen_oral') {
      setPendingOral(null);
      try { sessionStorage.removeItem('an_oral_context'); } catch (_) {}
    }
    setActive(finalTarget);
    if (role === 'student' && !opts.fromPop) {
      const tabs = {
        course: finalTarget === 'mi_curso' && resolvedTab ? resolvedTab : studentCourseTab,
        evaluations: finalTarget === 'evaluaciones' && resolvedTab ? resolvedTab : studentEvalTab,
        documents: finalTarget === 'documentos_ayuda' && resolvedTab ? resolvedTab : studentDocsTab,
      };
  ...
  const cambiarPestanaEstudianteF984 = (section, tab) => {
    if (section === 'mi_curso') setStudentCourseTab(tab);
    if (section === 'evaluaciones') setStudentEvalTab(tab);
    if (section === 'documentos_ayuda') setStudentDocsTab(tab);
    if (role === 'student') {
      const tabs = {
        course: section === 'mi_curso' ? tab : studentCourseTab,
        evaluations: section === 'evaluaciones' ? tab : studentEvalTab,
        documents: section === 'documentos_ayuda' ? tab : studentDocsTab,
      };
      try { history.pushState({ anCampus:true, route:section, tabs }, '', studentRouteHashF984(section, tabs)); } catch (_) {}
    }
  };
  ...
  useEffect(() => {
    ...
    const tabsIniciales = { course:'cronograma', evaluations:'proximas', documents:'programa' };
    try { history.replaceState({ anCampus:true, route:'dashboard', tabs:tabsIniciales }, '', '#dashboard'); } catch (_) {}
    const onPop = (ev) => {
      const st = ev.state || {};
  ...
  if (role === 'student') {
    const map = {
      ...
      evaluaciones: <LazyRoute title="Evaluaciones" component="StudentEvaluationsView" files={F96_LAZY.student_evaluations}
        initialTab={studentEvalTab} onTabChange={(tab)=>cambiarPestanaEstudianteF984('evaluaciones', tab)}
        onNavigate={navigateTo} renderUpcoming={() => <ExamenesStudentPanel />} />,
    ...
    const map = {
      ...
      prematriculas: <LazyRoute title="Prematrículas" component="FreeUserRequestsAdminView" files={F96_LAZY.free_user_admin} toast={toast} />,
      solicitudes_prematricula: <LazyRoute title="Prematrículas" component="FreeUserRequestsAdminView" files={F96_LAZY.free_user_admin} toast={toast} />,
