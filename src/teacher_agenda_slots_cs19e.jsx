// F98.4-Z6-CS19E · Agenda docente core-ready por slots reales + modo Club I CAN aislado
...
(function(){
  ...
  function injectAgendaSlotsStyleCS19E(){
    if (typeof document === 'undefined' || document.getElementById('an-cs19e-teacher-agenda-style')) return;
    const style = document.createElement('style');
    style.id = 'an-cs19e-teacher-agenda-style';
    style.textContent = `
      .cs19e-agenda-card{
      ...
      .cs19e-agenda-card:hover{ transform: translateY(-1px); }
      .cs19e-agenda-card:active{ transform: translateY(0); }
      .cs19e-agenda-card:focus-visible{ outline:3px solid rgba(0,59,120,.24); outline-offset:2px; }
      .cs19e-agenda-card *{ pointer-events:none; }
      .cs19e-agenda-row{ min-height:78px; display:grid; align-content:start; gap:7px; }
      .cs19e-agenda-row-empty{ min-height:78px; border:1px dashed rgba(15,23,42,.07); border-radius:12px; background:rgba(15,23,42,.015); }
      @media (prefers-reduced-motion: reduce){ .cs19e-agenda-card{ transition:none; } .cs19e-agenda-card:hover{ transform:none; } }
      @media (max-width: 860px){ .cs19e-agenda-card{ min-width:118px; } }
    `;
    document.head.appendChild(style);
  }
  injectAgendaSlotsStyleCS19E();
  ...
  function MisGruposSwitcherCS19E({ grupos, activo, onSelect, activeSession }) {
    ...
    const Card = ({ it, idx }) => {
      ...
      return <button className="cs19e-agenda-card" key={`${cod}-${riel}-${slot.day}-${slot.hora_i || idx}-${slot.hora_f || ''}`} type="button" onClick={() => onSelect && onSelect(cod)} title={title} style={{
              ...
              return (
                <div key={day.key} style={{ minHeight:194, borderRight:'1px solid var(--line)', borderBottom:'1px solid var(--line)', background:'#FFF' }}>
                  ...
                  <div style={{ display:'grid', gridTemplateRows:'minmax(78px, auto) minmax(78px, auto)', gap:8, padding:8 }}>
                    <div className="cs19e-agenda-row">
                      {topItems.length ? topItems.map((it, idx) => <Card key={`top-${itemKey(it)}-${idx}`} it={it} idx={idx}/>) : <div className="cs19e-agenda-row-empty" aria-hidden="true" />}
                    </div>
                    <div className="cs19e-agenda-row">
                      {bottomItems.length ? bottomItems.map((it, idx) => <Card key={`bottom-${itemKey(it)}-${idx}`} it={it} idx={idx}/>) : <div className="cs19e-agenda-row-empty" aria-hidden="true" />}
                    </div>
                  </div>
                </div>
              );
  ...
  function tvIcanSlotsF82CS19E(g){ return slotsFor(g, 'ican'); }
  function tvIcanDayIndexesF82CS19E(g){ return slotsFor(g, 'ican').map(s => s.day); }
  function tvIcanStartMinutesF82CS19E(g){ const ss = slotsFor(g, 'ican'); return ss.length ? ss[0].start : 9999; }
  function tvIcanHoraLabelF82CS19E(g){
  ...
  function installViewModeWrappersCS19E(){
    if (window.__AN_TEACHER_AGENDA_VIEW_MODE_WRAPPED_CS19E) return;
    window.__AN_TEACHER_AGENDA_VIEW_MODE_WRAPPED_CS19E = true;
    ...
    if (typeof OriginalGrupos === 'function') {
      window.GruposView = function GruposViewCS19E(props){
        setAgendaMode('all');
    ...
    if (typeof OriginalCronograma === 'function') {
      window.CronogramaDocenteSeguroF82 = function CronogramaDocenteSeguroCS19E(props){
    ...
    window.ClubICANDocenteView = function ClubICANDocenteViewCS19E(props){
      setAgendaMode('ican');
  ...
  window.__AN_TEACHER_AGENDA_SLOTS_VERSION = 'F98.4-Z6-CS19E';
  window.anTeacherAgendaSlotsDebugCS19E = function(grupos){
    const lista = Array.isArray(grupos) ? grupos : [];
    ...
    return { version:'F98.4-Z6-CS19E', mode:currentAgendaMode(), dias:out };
  };
  window.anTeacherAgendaSlotsDebug = window.anTeacherAgendaSlotsDebugCS19E;

  try { window.addEventListener('hashchange', () => { if (!/ican|club/.test(String(location.hash || '').toLowerCase())) setAgendaMode('all'); }); } catch(_) {}
  installViewModeWrappersCS19E();
  window.MisGruposSwitcher = MisGruposSwitcherCS19E;
  window.tvIcanSlotsF82CS19E = tvIcanSlotsF82CS19E;
  window.tvIcanDayIndexesF82 = tvIcanDayIndexesF82CS19E;
  window.tvIcanStartMinutesF82 = tvIcanStartMinutesF82CS19E;
  window.tvIcanHoraLabelF82 = tvIcanHoraLabelF82CS19E;
  try { MisGruposSwitcher = MisGruposSwitcherCS19E; } catch(_) {}
  try { tvIcanDayIndexesF82 = tvIcanDayIndexesF82CS19E; } catch(_) {}
  try { tvIcanStartMinutesF82 = tvIcanStartMinutesF82CS19E; } catch(_) {}
  try { tvIcanHoraLabelF82 = tvIcanHoraLabelF82CS19E; } catch(_) {}
})();
