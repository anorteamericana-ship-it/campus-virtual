...
(function () {
  ...
  core.openingMarkers = function (group, weekStart) {
    ...
    const result = [];
    for (let i = 0; i < 6; i += 1) {
      const date = core.addDays(weekStart, i);
      if (date <= end && allowed.includes(date.getDay())) result.push({ leccion: 0, fecha: core.isoDate(date), tipo: 'APERTURA', estado: 'PROYECTADO' });
    }
    return result;
  };
})();