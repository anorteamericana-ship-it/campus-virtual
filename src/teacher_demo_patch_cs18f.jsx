// F98.4-Z6-CS18F · Teacher agenda switcher two-row morning/night
// Corrige la banda superior semanal: mañana arriba y noche abajo.
...
(function(){
  ...
  function MisGruposSwitcherCS18F({ grupos, activo, onSelect, activeSession }) {
    ...
    const Card = ({ it, idx }) => {
      ...
      return <button key={`${cod}-${riel}-${slot.day}-${slot.hora_i || idx}-${slot.hora_f || ''}`} type="button" onClick={() => onSelect && onSelect(cod)} title={title} style={{
        ...
        position:'relative', minHeight:nextHere ? 78 : 68, width:'100%'
      }}>
        <div style={{ fontSize:10.5, fontWeight:900, color:isIcan ? dark : 'var(--ink)', lineHeight:1.15 }}>{daysLabel}</div>
        <div style={{ fontSize:15, fontWeight:900, color:isIcan ? dark : 'var(--an-navy)', lineHeight:1.1, marginTop:3, whiteSpace:'nowrap' }}>{hourLabel}</div>
        ...
        <div style={{ overflowX:'auto', WebkitOverflowScrolling:'touch' }}>
          <div style={{ minWidth:980, display:'grid', gridTemplateColumns:'repeat(7, minmax(128px, 1fr))', borderTop:'1px solid var(--line)', borderLeft:'1px solid var(--line)' }}>
            {WEEK.map(day => {
              ...
              return (
                <div key={day.key} style={{ minHeight:160, borderRight:'1px solid var(--line)', borderBottom:'1px solid var(--line)', background:'#FFF' }}>
                  ...
                  <div style={{ display:'grid', gridTemplateRows:'minmax(68px, auto) minmax(68px, auto)', gap:7, padding:7 }}>
                    <div style={{ display:'grid', alignContent:'start', gap:7, minHeight:68 }}>
  ...
  function tvIcanSlotsF82CS18F(g){ return slotsFor(g, 'ican'); }
  function tvIcanDayIndexesF82CS18F(g){ return slotsFor(g, 'ican').map(s => s.day); }
  function tvIcanStartMinutesF82CS18F(g){ const ss = slotsFor(g, 'ican'); return ss.length ? ss[0].start : 9999; }
  function tvIcanHoraLabelF82CS18F(g){
    const ss = slotsFor(g, 'ican');
    if (!ss.length) return '';
    return ss.map(s => `${s.shortDay} ${s.hourLabel}`).join(' / ');
  }

  window.MisGruposSwitcher = MisGruposSwitcherCS18F;
  window.tvIcanSlotsF82CS18F = tvIcanSlotsF82CS18F;
  window.tvIcanDayIndexesF82 = tvIcanDayIndexesF82CS18F;
  window.tvIcanStartMinutesF82 = tvIcanStartMinutesF82CS18F;
  window.tvIcanHoraLabelF82 = tvIcanHoraLabelF82CS18F;
  try { MisGruposSwitcher = MisGruposSwitcherCS18F; } catch(_) {}
  try { tvIcanDayIndexesF82 = tvIcanDayIndexesF82CS18F; } catch(_) {}
  try { tvIcanStartMinutesF82 = tvIcanStartMinutesF82CS18F; } catch(_) {}
  try { tvIcanHoraLabelF82 = tvIcanHoraLabelF82CS18F; } catch(_) {}
})();
