...
function AplicarPago() {
  ...
  const Paso1 = () => {
    ...
    const buscar = async () => {
      ...
      try {
        ...
        setEstSel(data.estudiante);
        setEstData({ niveles: data.niveles || {}, pagos: data.pagos || [], otrosPagos: data.otrosPagos || [], grupo: data.grupo || '', pendientes: data.pendientes || {} });
        setError('');
        setPaso(2);
      } catch(e) {
  ...
  const Paso2 = () => {
    ...
    const cert      = pendientes.certificado   || 0;
      ...
      return (
        <div style={{ textAlign:'center' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
            ...
            <button onClick={()=>{ setComprSel(null); setConfirmado(null); setTotalAplicarPanel(0); setPaso(3); }} className="btn btn-primary" style={{ background:'var(--an-granate)', borderColor:'var(--an-granate)', padding:14 }}>
              Otro comprobante
            </button>
          </div>
        </div>
