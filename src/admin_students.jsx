  ...
  return <div style={{padding:'14px 16px 22px',background:'var(--bg,#f7f4ef)'}}>
    ...
    <div style={{background:'linear-gradient(135deg,#112A4C,#193C68)',color:'white',borderRadius:14,padding:'14px 16px',marginBottom:12,boxShadow:'0 10px 24px rgba(20,33,61,.16)'}}>
      <div style={{display:'flex',justifyContent:'space-between',gap:14,flexWrap:'wrap',alignItems:'flex-start'}}>
        ...
        <div style={{display:'flex',gap:7,alignItems:'stretch',flexWrap:'wrap'}}><div style={{minWidth:110,padding:'8px 10px',borderRadius:9,background:'rgba(255,255,255,.1)',border:'1px solid rgba(255,255,255,.22)',textAlign:'center'}}><div style={{fontSize:8.5,fontWeight:900,textTransform:'uppercase',letterSpacing:'.1em',opacity:.72}}>Asistencia total</div><div style={{marginTop:2,fontSize:21,fontWeight:900,color:pctGlobal==null?'white':(pctGlobal>=70?'#BDE9C4':'#FFC4C0')}}>{pctGlobal==null?'—':`${pctGlobal}%`}</div><div style={{fontSize:8.5,opacity:.72}}>{totalAsis?`${totalPres}/${totalAsis} registros`:'Sin registros'}</div></div><button type="button" onClick={()=>setHistorial(true)} style={{padding:'8px 11px',borderRadius:9,border:'1px solid rgba(255,255,255,.24)',background:'rgba(255,255,255,.1)',color:'white',fontWeight:900,cursor:'pointer'}}>🕘 Historial</button><button type="button" onClick={onClose} style={{padding:'8px 11px',borderRadius:9,border:'1px solid rgba(255,255,255,.24)',background:'white',color:'#14213D',fontWeight:900,cursor:'pointer'}}>Cerrar</button></div>
      </div>
    </div>
