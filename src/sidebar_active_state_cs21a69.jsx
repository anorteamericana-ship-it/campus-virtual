...
(function () {
  ...
  function navButtons(aside) {
    return Array.from(aside?.querySelectorAll?.('button.sb-item') || []);
  }
  ...
  function findByKey(aside, key) {
    if (!key) return null;
    return navButtons(aside).find(button => usable(button) && buttonKey(button) === key) || null;
  }

  function findByLabels(aside, labels) {
    const wanted = (labels || []).map(value => clean(value).toLowerCase());
    return navButtons(aside).find(button => usable(button) && wanted.includes(menuLabel(button).toLowerCase())) || null;
  }
  ...
  function currentRoute(role) {
    try {
      return clean(
        localStorage.getItem(`an_active_${role}`) ||
        localStorage.getItem('an_active') ||
        ''
      );
    } catch (_) {
      return '';
  ...
  function routeCandidate(aside, role) {
    ...
    if (role === 'teacher' && route === 'materiales') {
      ...
      const candidate = findByLabels(aside, labels);
      if (candidate) return candidate;
    }

    if (role === 'student' && (route === 'mi_curso' || route === 'materiales')) {
      const candidate = findByLabels(aside, [mode === 'additional' ? 'Recursos adicionales' : 'Libros y Audios']);
      if (candidate) return candidate;
    }
    ...
    const byId = navButtons(aside).find(button => usable(button) && clean(button.getAttribute('data-nav-id')) === route);
    if (byId) return byId;
    ...
    return findByLabels(aside, routeLabels);
  }

  function applySingle(aside, target) {
    if (!aside) return;
    navButtons(aside).forEach(button => {
      const shouldBeActive = button === target;
      if (button.classList.contains('active') !== shouldBeActive) {
        button.classList.toggle('active', shouldBeActive);
      }
      if (shouldBeActive) button.setAttribute('aria-current', 'page');
  ...
  function SidebarActiveGuard({ role, revision }) {
    React.useEffect(() => {
      ...
      const onClick = event => {
        ...
        const button = event.target?.closest?.('aside button.sb-item');
        if (!button || !usable(button)) return;
        const owner = button.closest('aside');
        if (!owner || owner !== aside) aside = owner;
        ...
        schedule(key);
        setTimeout(() => schedule(key), 40);
        setTimeout(() => schedule(key), 180);
        setTimeout(() => schedule(), 500);
      };

      document.addEventListener('click', onClick, true);
      if (!attach()) attachTimer = window.setInterval(() => { if (attach()) window.clearInterval(attachTimer); }, 60);
  ...
  function install() {
    ...
    const Wrapped = function SidebarCS21A69(props) {
      ...
      React.useEffect(() => {
        ...
        const events = [
          'an:teacher-material-tab',
          'an:admin-resource-tab',
          RESOURCE_MODE_EVENT,
          'an:session-changed',
          'hashchange',
          'popstate',
      ...
      const role = normalizeRole(props?.role || props?.rolReal);
      return (
        <>
          <Base {...props} __cs21a69Revision={revision} />
          <SidebarActiveGuard role={role} revision={revision} />
        </>
      );
    };
