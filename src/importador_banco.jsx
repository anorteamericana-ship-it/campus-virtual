...
// F98.4-Z6-CS21A124 · Importador BCR: los ajustes internos no bloquean créditos nuevos.
...
function Bank123Badge({ movement }) {
  const states={NUEVO:['NUEVO','#2E7D32','#EAF5EC'],YA_EXISTE:['YA EXISTE','#667085','#EEF0F2'],DUPLICADO_ARCHIVO:['REPETIDO EN ARCHIVO','#795548','#F4ECE8'],CONFLICTO:['AJUSTE INTERNO','#B54708','#FFF2E5'],CONFLICTO_ARCHIVO:['REVISAR ARCHIVO','#B42318','#FEECEB'],DEBITO:['DÉBITO','#C00000','#FDECEC'],INVALIDO:['INVÁLIDO','#C00000','#FDECEC']};
...
function ImportadorBancario() {
  ...
  return <div>
    <PageHeader kicker="Conciliación bancaria" title={<>Importar <em>extracto BCR</em></>} sub="Compara cada documento contra toda BDBANCARIO y permite importar únicamente los créditos realmente nuevos" />
    ...
    {step===2&&<div>
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(145px,1fr))',gap:10,marginBottom:16}}>{[['Movimientos',movements.length],['Nuevos créditos',newMovements.length],['Ya existen',existingMovements.length],['Ajustes internos',conflicts.length],['Débitos',debits.length]].map(([label,count])=><div key={label} style={{padding:'12px 14px',border:'1px solid var(--line)',borderRadius:13,background:'var(--surface)'}}><div style={{fontSize:9.5,fontWeight:900,letterSpacing:'.1em',color:'var(--ink-3)',textTransform:'uppercase'}}>{label}</div><div style={{fontFamily:'var(--f-serif)',fontSize:27,color:label==='Nuevos créditos'?'#2E7D32':label==='Ajustes internos'&&count?'#B54708':'var(--an-navy)',marginTop:2}}>{count}</div></div>)}</div>
      {conflicts.length>0&&<div style={{padding:'12px 14px',marginBottom:12,border:'1px solid #E6A75C',borderRadius:12,background:'#FFF7ED',color:'#8A4B08',fontSize:12}}><strong>{conflicts.length} movimiento{conflicts.length===1?'':'s'} ya registrado{conflicts.length===1?'':'s'} tiene{conflicts.length===1?'':'n'} un monto diferente.</strong> Se conserva el valor existente de APOLLO, se excluye{conflicts.length===1?'':'n'} de esta importación y no bloquea{conflicts.length===1?'':'n'} los créditos nuevos.</div>}
