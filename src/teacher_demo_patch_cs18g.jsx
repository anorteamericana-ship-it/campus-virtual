// F98.4-Z6-CS18G · Teacher agenda polish: colores, simetría y cursor seguro
// Mantiene la regla visual CS18F: mañana arriba y noche abajo.
// No escribe datos. Solo mejora banda superior semanal y estados visuales.
/* global React */
(function(){
  ...
  function injectAgendaPolishStyleCS18G(){
    if (typeof document === 'undefined' || document.getElementById('an-cs18g-teacher-agenda-style')) return;
    const style = document.createElement('style');
    style.id = 'an-cs18g-teacher-agenda-style';
    style.textContent = `
      .cs18g-agenda-card{
      ...
      .cs18g-agenda-card:hover{ transform: translateY(-1px); }
      .cs18g-agenda-card:active{ transform: translateY(0); }
      .cs18g-agenda-card:focus-visible{ outline:3px solid rgba(0,59,120,.24); outline-offset:2px; }
      .cs18g-agenda-card *{ pointer-events:none; }
      .cs18g-agenda-row{ min-height:78px; display:grid; align-content:start; gap:7px; }
      @media (max-width: 860px){ .cs18g-agenda-card{ min-width:118px; } }
    `;
    document.head.appendChild(style);
  }
  injectAgendaPolishStyleCS18G();
  ...
  function buildWeeklyItems(lista){
  ...
  function MisGruposSwitcherCS18G({ grupos, activo, onSelect, activeSession }) {
    const lista = Array.isArray(grupos) ? grupos : [];
    const byDay = buildWeeklyItems(lista);
    ...
    const Card = ({ it, idx }) => {
      ...
      return <button className="cs18g-agenda-card" key={`${cod}-${riel}-${slot.day}-${slot.hora_i || idx}-${slot.hora_f || ''}`} type="button" onClick={() => onSelect && onSelect(cod)} title={title} style={{
              ...
              return (
                <div key={day.key} style={{ minHeight:194, borderRight:'1px solid var(--line)', borderBottom:'1px solid var(--line)', background:'#FFF' }}>
                  ...
                  <div style={{ display:'grid', gridTemplateRows:'minmax(78px, auto) minmax(78px, auto)', gap:8, padding:8 }}>
                    <div className="cs18g-agenda-row">
                      {topItems.map((it, idx) => <Card key={`top-${itemKey(it)}-${idx}`} it={it} idx={idx}/>)}
                    </div>
                    <div className="cs18g-agenda-row">
                      {bottomItems.map((it, idx) => <Card key={`bottom-${itemKey(it)}-${idx}`} it={it} idx={idx}/>)}
                    </div>
                  </div>
                </div>
              );
  ...
  function tvIcanSlotsF82CS18G(g){ return slotsFor(g, 'ican'); }
  function tvIcanDayIndexesF82CS18G(g){ return slotsFor(g, 'ican').map(s => s.day); }
  function tvIcanStartMinutesF82CS18G(g){ const ss = slotsFor(g, 'ican'); return ss.length ? ss[0].start : 9999; }
  function tvIcanHoraLabelF82CS18G(g){
    const ss = slotsFor(g, 'ican');
    if (!ss.length) return '';
    return ss.map(s => `${s.shortDay} ${s.hourLabel}`).join(' / ');
  }

  window.MisGruposSwitcher = MisGruposSwitcherCS18G;
  window.tvIcanSlotsF82CS18G = tvIcanSlotsF82CS18G;
  window.tvIcanDayIndexesF82 = tvIcanDayIndexesF82CS18G;
  window.tvIcanStartMinutesF82 = tvIcanStartMinutesF82CS18G;
  window.tvIcanHoraLabelF82 = tvIcanHoraLabelF82CS18G;
  try { MisGruposSwitcher = MisGruposSwitcherCS18G; } catch(_) {}
  try { tvIcanDayIndexesF82 = tvIcanDayIndexesF82CS18G; } catch(_) {}
  try { tvIcanStartMinutesF82 = tvIcanStartMinutesF82CS18G; } catch(_) {}
  try { tvIcanHoraLabelF82 = tvIcanHoraLabelF82CS18G; } catch(_) {}
})();
