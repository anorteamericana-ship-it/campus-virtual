/* F98.4-Z6-CS21A20G · Tabla de leads ordenable + acción CONAPE por fila. */
(function(){
  const COLS=[
  ...
  function SortableProspectoTable({lista,onOpen,onChanged,onToast}){
    ...
    const rows=React.useMemo(()=>{const base=(Array.isArray(lista)?lista:[]).map((p,i)=>({p,i}));if(!sort.key)return base.map(x=>x.p);return base.sort((a,b)=>compareRows(a,b,sort.key,sort.dir)).map(x=>x.p);},[lista,sort]);
    ...
    return <div className="vx-tablecard"><div className="vx-table-scroll"><table className="vx-table">
      ...
      <tbody>{rows.map((p,i)=>{const dias=window.diasDesde(p.fecha_registro),prio=window.calcularPrioridadProspecto(p),est=window.calcularEstadoEstudianteVentas(p),ced=digits(p.cedula),etapa=text(conapeEtapas[ced]||etapaVisible(p));return <tr key={p.cedula||p.id||i} className={prio.nivel==='rojo'?'vx-row-rojo':''} onClick={()=>onOpen(p)}>
