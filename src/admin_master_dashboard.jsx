...
function MasterProductionSmoke(){
  ...
  const run=async()=>{setState(s=>({...s,loading:true,error:''}));try{const data=await masterAction('ejecutarSmokeTestSeguimiento',{frontend_version:MASTER_PANEL_BUILD});setState({loading:false,error:'',data});await loadRegistry();}catch(e){setState({loading:false,error:masterSafeUserError(e?.message || String(e), 'No se pudo completar el control de publicación. Intentá de nuevo.', 'control_publicacion'),data:null});}};
