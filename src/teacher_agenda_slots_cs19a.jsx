// F98.4-Z6-CS19A · Agenda docente por slots para todos los profesores
// Integra la corrección visual CS18G sin clonar datos demo.
// Cada docente ve sus grupos reales; Keylor solo usa su máscara cuando el backend demo está activo.
// Mantiene la regla visual: mañana arriba, noche abajo, colores por tipo/nivel.
/* global React */
(function(){
  ...
  function injectAgendaSlotsStyleCS19A(){
    if (typeof document === 'undefined' || document.getElementById('an-cs19a-teacher-agenda-style')) return;
    const style = document.createElement('style');
    style.id = 'an-cs19a-teacher-agenda-style';
    style.textContent = `
      .cs19a-agenda-card{
      ...
      .cs19a-agenda-card:hover{ transform: translateY(-1px); }
      .cs19a-agenda-card:active{ transform: translateY(0); }
      .cs19a-agenda-card:focus-visible{ outline:3px solid rgba(0,59,120,.24); outline-offset:2px; }
      .cs19a-agenda-card *{ pointer-events:none; }
      .cs19a-agenda-row{ min-height:78px; display:grid; align-content:start; gap:7px; }
      @media (max-width: 860px){ .cs19a-agenda-card{ min-width:118px; } }
    `;
    document.head.appendChild(style);
  }
  injectAgendaSlotsStyleCS19A();
  ...
  function MisGruposSwitcherCS19A({ grupos, activo, onSelect, activeSession }) {
    ...
    const Card = ({ it, idx }) => {
      ...
      return <button className="cs19a-agenda-card" key={`${cod}-${riel}-${slot.day}-${slot.hora_i || idx}-${slot.hora_f || ''}`} type="button" onClick={() => onSelect && onSelect(cod)} title={title} style={{
              ...
              return (
                <div key={day.key} style={{ minHeight:194, borderRight:'1px solid var(--line)', borderBottom:'1px solid var(--line)', background:'#FFF' }}>
                  ...
                  <div style={{ display:'grid', gridTemplateRows:'minmax(78px, auto) minmax(78px, auto)', gap:8, padding:8 }}>
                    <div className="cs19a-agenda-row">
  ...
  function tvIcanSlotsF82CS19A(g){ return slotsFor(g, 'ican'); }
  function tvIcanDayIndexesF82CS19A(g){ return slotsFor(g, 'ican').map(s => s.day); }
  function tvIcanStartMinutesF82CS19A(g){ const ss = slotsFor(g, 'ican'); return ss.length ? ss[0].start : 9999; }
  function tvIcanHoraLabelF82CS19A(g){
    const ss = slotsFor(g, 'ican');
    if (!ss.length) return '';
    return ss.map(s => `${s.shortDay} ${s.hourLabel}`).join(' / ');
  }

  window.MisGruposSwitcher = MisGruposSwitcherCS19A;
  window.tvIcanSlotsF82CS19A = tvIcanSlotsF82CS19A;
  window.tvIcanDayIndexesF82 = tvIcanDayIndexesF82CS19A;
  window.tvIcanStartMinutesF82 = tvIcanStartMinutesF82CS19A;
  window.tvIcanHoraLabelF82 = tvIcanHoraLabelF82CS19A;
  try { MisGruposSwitcher = MisGruposSwitcherCS19A; } catch(_) {}
  try { tvIcanDayIndexesF82 = tvIcanDayIndexesF82CS19A; } catch(_) {}
  try { tvIcanStartMinutesF82 = tvIcanStartMinutesF82CS19A; } catch(_) {}
  try { tvIcanHoraLabelF82 = tvIcanHoraLabelF82CS19A; } catch(_) {}
})();
