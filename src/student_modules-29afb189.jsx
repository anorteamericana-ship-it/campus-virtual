...
function PagosView({ tweaks = {} }) {
  ...
  const [expandedId, setExpandedId] = React.useState(() => {
    const cur = LEVEL_FINANCE.find(l => l.status === 'current');
    return cur ? cur.id : LEVEL_FINANCE[0].id;
  });

  // global status — flattens all levels for the hero
  const allCuotas = LEVEL_FINANCE.flatMap(l => l.cuotas);
  const allConcepts = LEVEL_FINANCE.flatMap(l => [l.matricula, ...l.cuotas, l.certificado]);
  ...
  const currentLevel = LEVEL_FINANCE.find(l => l.status === 'current');
      ...
      <div className="card" style={{
        ...
        <div style={{ display:'grid', gridTemplateColumns:'auto 1fr auto', gap:24, alignItems:'center', position:'relative' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:8, alignItems:'flex-end' }}>
            ...
            <div style={{ fontSize:11, color:'var(--ink-3)' }}>
              {LEVEL_FINANCE.filter(l => l.status === 'completed').length + (currentLevel ? 1 : 0)} / {LEVEL_FINANCE.length} niveles iniciados
            </div>
          </div>
        </div>
      </div>
...
function PerfilView({ onNavigate } = {}) {
  const s = STUDENT;
  const schedule = typeof buildGroupSchedule === 'function' && typeof DEMO_GROUP !== 'undefined'
    ? buildGroupSchedule('b1', DEMO_GROUP, typeof DEMO_SUSPENSIONS !== 'undefined' ? DEMO_SUSPENSIONS : [])
    : [];
  const TODAY_P = new Date('2026-04-18T00:00:00');
  return (
    <div>
      <PageHeader
        kicker="Mi cuenta"
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
          ...
          <div className="card">
            <div className="card-h">
              ...
              <Chip tone="gold">3 desbloqueadas</Chip>
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(6, 1fr)', gap:12 }}>
              {[
                { t:'Primera clase', e:true },
                { t:'100% asistencia', e:true },
                { t:'Oral Test 1', e:true },
                { t:'Written Test 1', e:false },
                { t:'Módulo completo', e:false },
                { t:'8 I CAN', e:false },
              ].map((b, i) => (
                <div key={i} className={`badge ${b.e?'earned':''}`}>
                  <div className="badge-num">{b.e ? '★' : '—'}</div>
                  <div className="badge-label">{b.t}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      {onNavigate && schedule.length > 0 && (
        <PerfilMiniCalendar schedule={schedule} today={TODAY_P} onOpenLesson={(n) => onNavigate('materiales', { lesson: n })} />
      )}
    </div>
  );
}

function PerfilMiniCalendar({ schedule, today, onOpenLesson }) {
  ...
  const byDate = {};
  schedule.forEach(s => { const k = s.date.toDateString(); (byDate[k] = byDate[k] || []).push(s); });
  const nav = (delta) => { let m = month + delta, y = year; if (m<0){m=11;y--;} if (m>11){m=0;y++;} setMonth(m); setYear(y); };
  const nextLessons = schedule.filter(s => s.date >= today).slice(0, 4);
  return (
    <div className="card" style={{ marginTop: 20, padding: 20 }}>
      <div className="card-h" style={{ marginBottom: 14 }}>
        <div>
          <div className="card-title">Mi Calendario</div>
          <div style={{ fontSize: 11, color: 'var(--ink-3)', marginTop: 2 }}>Clic en una lección para ver sus materiales →</div>
        </div>
        <div style={{ display:'flex', gap:6, alignItems:'center' }}>
          <button className="btn btn-ghost" onClick={() => nav(-1)} style={{ fontSize:11 }}>←</button>
          <div style={{ fontFamily:'var(--f-serif)', fontSize:16, fontWeight:500, minWidth:140, textAlign:'center', textTransform:'capitalize' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr', gap:16 }}>
        <div>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(7, 1fr)', gap:3, marginBottom:3 }}>
            {['D','L','M','X','J','V','S'].map((d,i) => (
              <div key={i} style={{ fontSize:10, fontWeight:700, color:'var(--ink-3)', textAlign:'center', padding:'2px 0' }}>{d}</div>
            ))}
          </div>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(7, 1fr)', gap:3 }}>
            {cells.map((d, i) => {
              if (!d) return <div key={i} />;
              const lessons = byDate[d.toDateString()] || [];
              const isToday = d.toDateString() === today.toDateString();
              const l = lessons[0];
              return (
                <button key={i} onClick={() => l && onOpenLesson(l.n)} disabled={!l} style={{ minHeight:44, padding:4, background: isToday ? 'color-mix(in srgb, var(--an-granate) 8%, white)' : l ? 'var(--surface)' : 'transparent', border: isToday ? '2px solid var(--an-granate)' : '1px solid var(--line)', borderRadius:5, cursor: l ? 'pointer' : 'default', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:2, opacity: (!l && d < today) ? 0.4 : 1 }}>
                  <div style={{ fontSize:10, fontWeight:isToday?700:500, color: isToday?'var(--an-granate)':'var(--ink-2)' }}>{d.getDate()}</div>
                  {l && <div style={{ fontSize:8, padding:'1px 4px', borderRadius:3, fontWeight:700, background: l.date < today ? 'var(--ok)' : isToday ? 'var(--an-granate)' : 'var(--an-navy)', color:'white' }}>L{l.n}</div>}
                </button>
              );
            })}
          </div>
        ...
        <div>
          <div style={{ fontSize:10, fontWeight:700, letterSpacing:'0.1em', textTransform:'uppercase', color:'var(--ink-3)', marginBottom:6 }}>Próximas</div>
          {nextLessons.map(l => (
            <button key={l.n} onClick={() => onOpenLesson(l.n)} style={{ width:'100%', textAlign:'left', padding:'8px 10px', marginBottom:6, background:'var(--surface-2)', border:'1px solid var(--line)', borderRadius:6, cursor:'pointer' }}>
              <div style={{ display:'flex', gap:6, alignItems:'baseline' }}>
                <span style={{ fontFamily:'var(--f-mono)', fontSize:9, color:'var(--ink-3)', fontWeight:700 }}>L{String(l.n).padStart(2,'0')}</span>
                <span style={{ fontSize:12, fontWeight:600, color:'var(--ink)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{l.title}</span>
              </div>
              <div style={{ fontSize:10, color:'var(--ink-2)', marginTop:2 }}>{typeof fmtDate==='function' ? fmtDate(l.date) : l.date.toLocaleDateString('es-CR')}</div>
            </button>
          ))}
        </div>
      </div>
