...
server = server.replace("const VERSION = 'V4.1.2';", "const VERSION = 'V4.1.3';", 1)
start = server.index('async function confirmInHome(p, sessionId, cedula) {')
end = server.index('\nasync function readProspectListPage(p) {', start)
new = r'''async function countIrFilters(p) {
  return p.evaluate(() => {
...
old_t = "create_count:Number(created?.createCount || 0), confirmation_found:!!confirmation?.found, confirmation_estado:upper(confirmation?.estado || ''),\n      fill_attempted_fields:"
new_t = "create_count:Number(created?.createCount || 0), confirmation_found:!!confirmation?.found, confirmation_estado:upper(confirmation?.estado || ''),\n      ir_filters_before:Number(confirmation?.ir_filters_before || 0), ir_reset_method:txt(confirmation?.ir_reset_method || 'NONE'), pages_scanned:Number(confirmation?.pages_scanned || 0), rows_scanned:Number(confirmation?.rows_scanned || 0),\n      fill_attempted_fields:"
if old_t not in server:
    raise SystemExit('STOP telemetry baseline')
server = server.replace(old_t, new_t, 1)
qa = qa.replace("CONAPE Bridge V4.1.2 QA PASS", "CONAPE Bridge V4.1.3 QA PASS", 1)
anchor = "  ['telemetría de confirmación reporta existencia y estado informativo', /confirmation_found/.test(server) && /confirmation_estado:upper\\(confirmation\\?\\.estado \\|\\| ''\\)/.test(server) && !/confirmation_registro/.test(server)],\n"
addition = anchor + "  ['confirmación IR limpia filtros y no usa sleep fijo de 900ms', /resetInteractiveReport/.test(confirmBlock) && /ACTIONS_RESET/.test(confirmBlock) && /CHIP_CLOSE/.test(confirmBlock) && /waitForApexDynamicAction/.test(confirmBlock) && !/sleep\\(900\\)/.test(confirmBlock)],\n  ['confirmación hace fallback paginado antes de no encontrado', /scanProspectPagesForCedula/.test(server) && /readProspectListPage/.test(server) && /clickProspectNextPage/.test(server) && /pages_scanned/.test(server) && /rows_scanned/.test(server)],\n  ['telemetría confirmación incluye filtros, reset, páginas y filas', ['ir_filters_before','ir_reset_method','pages_scanned','rows_scanned','ms_confirmation'].every(v => server.includes(v))],\n"
if anchor not in qa:
    raise SystemExit('STOP QA anchor')
qa = qa.replace(anchor, addition, 1)
