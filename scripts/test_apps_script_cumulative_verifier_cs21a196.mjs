...
const sourceFiles=[
  ...
  'apps_script_patches/99_CS21A196_MEMORY_MATCH_CONFLICT_RECONCILIATION_COMPLETO.gs',
];
...
const cache={
  get(key){if(key==='ELIVE176|QA-DB|QUESTION_BANK') return JSON.stringify(questionRows);return cacheMap.get(String(key))||null;},
...
new vm.Script(source,{filename:'CS21A144-CS21A196-QA-CUMULATIVE.gs'}).runInContext(context);
...
const expected=[
  ...
  'CS21A194-MM-LATENCY-SAFE-1','CS21A195-MM-CONVERGENCE-RELAY-1','CS21A196-MM-CONFLICT-RECONCILIATION-1',
];
...
assert.equal(result.version,'CS21A196-MM-CONFLICT-RECONCILIATION-1');
assert.equal(result.previous_version,'CS21A195-MM-CONVERGENCE-RELAY-1');
assert.equal(result.mutation_response_revision_matches_written_state,true);
assert.equal(result.domain_conflicts_keep_canonical_package,true);
assert.equal(result.rejected_mutations_not_published_to_room_relay,true);
assert.equal(context._cs21a195PublishResponseRelay_.__cs21a196RejectsNotShared,true);

console.log(JSON.stringify({
  ok:true,
  contract:'CS21A196_APPS_SCRIPT_CUMULATIVE_RUNTIME_VERIFIER',
  sources:sourceFiles.length,
  versions:expected,
  final_version:result.version,
  response_revision_coherent:true,
  rejected_mutations_not_shared:true,
},null,2));
