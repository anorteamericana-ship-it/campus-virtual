...
for(const path of Object.keys(pre)){
 ...
 if(exact && sha!==cand[path]) throw Error(`${path}: candidate blob mismatch ${sha}`);
 ...
 ]) if(s.includes(unsafe)) throw Error(`${path}: teacher raw projection reintroduced`);
 if(exact){
...
console.log(exact ? 'CS21A210BA exact reconstruction PASS' : 'CS21A210BA descendant-safe teacher contract PASS');