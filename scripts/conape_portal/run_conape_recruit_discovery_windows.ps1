...
$discoveryScript = Join-Path $scriptDir 'discover_recruit_form_c3_3.mjs'
if (-not (Test-Path -LiteralPath $discoveryScript)) { throw "No existe C3.3 discovery: $discoveryScript" }
...
$profilePath = Join-Path $env:TEMP "an-conape-recruit-$PID-$stamp"
...
$exitCode = 1
try {
  Write-Host 'C3.3 · abriendo CONAPE en perfil temporal aislado...'
  ...
  Write-Host ''
  Write-Host 'Inicie sesión normalmente. No abra DevTools y NO presione Enviar/Guardar.'
  Write-Host 'El script buscará el botón Reclutar, abrirá únicamente su primera vista y leerá la estructura del formulario.'
  Write-Host 'No imprime valores escritos, cookies, tokens ni inputs ocultos.'
  Write-Host ''
...
if ($exitCode -eq 0) {
  Write-Host ''
  Write-Host 'C3.3 discovery terminó en lectura solamente. Perfil temporal eliminado.'
  exit 0
}
Write-Error "C3.3 discovery terminó bloqueado con código $exitCode."
exit $exitCode
