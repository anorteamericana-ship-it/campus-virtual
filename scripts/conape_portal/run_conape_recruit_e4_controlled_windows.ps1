...
$e4Script = Join-Path $scriptDir 'recruit_e4_controlled_c3_4.mjs'
if (-not (Test-Path -LiteralPath $e4Script)) { throw "No existe C3.4 E4 harness: $e4Script" }
...
$profilePath = Join-Path $env:TEMP "an-conape-e4-$PID-$stamp"
...
try {
  ...
  Write-Host 'Use un prospecto REAL y autorizado que todavía no esté reclutado en CONAPE.'
  Write-Host 'Si visualmente CONAPE muestra duplicado/advertencia, NO escriba CREAR.'
  Write-Host ''

  & $node.Source $e4Script --profile $profilePath --timeout-ms ($TimeoutSeconds * 1000)
...
if ($exitCode -eq 0) {
  Write-Host ''
  Write-Host 'C3.4 E4 terminó. Revise el bloque C3.4 E4 RESULT antes de hacer cualquier otra operación.'
  exit 0
}
Write-Error "C3.4 E4 terminó bloqueado con código $exitCode. NO reintente CREATE automáticamente si el bloque indicó write_performed=true."
exit $exitCode
