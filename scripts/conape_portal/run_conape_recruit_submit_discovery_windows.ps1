...
$discoveryScript = Join-Path $scriptDir 'discover_recruit_submit_contract_c3_3.mjs'
if (-not (Test-Path -LiteralPath $discoveryScript)) { throw "No existe C3.3 submit discovery: $discoveryScript" }
...
$profilePath = Join-Path $env:TEMP "an-conape-submit-$PID-$stamp"
...
$exitCode = 1
try {
  Write-Host 'C3.3 · abriendo CONAPE para descubrir SOLO el contrato del botón Crear nuevo Prospecto...'
  ...
  Write-Host 'El script abrirá la página PROSPECTO, pero NO presionará Crear nuevo Prospecto.'
  Write-Host 'No imprime valores de campos, hidden inputs, cookies, tokens, sesión ni onclick crudo.'
  Write-Host ''
...
if ($exitCode -eq 0) {
  Write-Host ''
  Write-Host 'C3.3 submit discovery terminó en lectura solamente. Perfil temporal eliminado.'
  exit 0
}
Write-Error "C3.3 submit discovery terminó bloqueado con código $exitCode."
exit $exitCode
