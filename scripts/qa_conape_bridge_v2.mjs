...
const checks = [
  ...
  ['V4.2.5 deja EVE ausente en telemetría y no bloquea antes del clic', !/EVENT_CONTEXT_MISSING/.test(createBlock) && !/if \(!eventContext\.eve_id_present\)/.test(createBlock) && /const eventContext = await ensureEventContext\(p\)/.test(createBlock)],
