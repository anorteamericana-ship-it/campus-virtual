...
const checks = [
  ...
  ['V4.2.6 prioriza Home -> Reclutar con click real y DIRECT_URL solo fallback', /readSignedNavigationTelemetry/.test(server) && /CONAPE_FRIENDLY_HOME/.test(server) && server.includes("clickVisibleByLabel(p, /(^| )RECLUTAR( |$)/i)") && server.includes("return finish('HOME_CLICK_RECRUIT')") && server.includes("return finish('DIRECT_URL')") && server.indexOf("return finish('HOME_CLICK_RECRUIT')") < server.indexOf("return finish('DIRECT_URL')")],
  ...
  ['V4.2.6 telemetría de navegación expone entry path y firma sin checksum', ['entry_path','nav_signed_eve','nav_signed_pro','nav_has_cs','nav_eve_matches_env','nav_pro_matches_env'].every(v => executeTelemetryBlock.includes(v)) && !/\bcs\s*:|checksum/.test(executeTelemetryBlock)],
...
console.log(`CONAPE Bridge V4.2.6 QA PASS · ${checks.length}/${checks.length}`);