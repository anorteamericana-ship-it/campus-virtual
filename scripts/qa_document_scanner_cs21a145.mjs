...
const checks=[
  ...
  [scanner.includes("version:'CS21A145-2'"),'scanner revisión real-world v2'],
  ...
  [scanner.includes('const cannyPasses'),'detección multi-Canny para fotos reales'],
  [scanner.includes("'otsu_light'") && scanner.includes("'otsu_dark'"),'detección Otsu complementaria'],
  [scanner.includes('scoreQuad(') && scanner.includes('ID_ASPECT'),'scoring geométrico documento identidad'],
  [scanner.includes('approx.data32S'),'lectura segura de vértices OpenCV.js'],
  [scanner.includes('requiresRetake: score < 60'),'calidad baja fuerza repetición'],
  [scanner.includes("detectionMethod:'none'") || scanner.includes("detectionMethod:normalized.detectionMethod"),'método de detección trazable'],
