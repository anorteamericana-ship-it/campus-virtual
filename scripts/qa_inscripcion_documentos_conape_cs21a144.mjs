...
ok(uploaderBlock.includes("accept={allowPdf?'image/jpeg,image/png,image/gif,image/webp,application/pdf':'image/jpeg,image/png,image/gif,image/webp'}"), 'input de archivos existe sin capture');
ok(uploaderBlock.includes('function FilePdf('), 'uploader de PDF de identidad existe');
ok(uploaderBlock.includes('accept="application/pdf"'), 'PDF limita selector a application/pdf');
ok(uploaderBlock.includes("file.size > 6 * 1024 * 1024"), 'PDF tiene límite preventivo 6 MB');
...
ok(docsBlock.includes('allowPdf'), 'título/último grado permite PDF además de imagen');
...
console.log('CS21A144 static QA PASS');
