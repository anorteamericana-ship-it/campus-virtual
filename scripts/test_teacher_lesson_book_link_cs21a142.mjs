...
assert.match(attendanceBridge, /teacher_views\.jsx\?v=[^'"\s]+/);
assert.equal((app.match(/teacher_views\.jsx\?v=[^'"\s]+/g) || []).length, 2);
assert.match(campus, /resources_panel_state_cs21a65\.js\?v=F98\.4Z6CS21A142/);
assert.match(campus, /att77_bridge\.js\?v=[^'"\s]+/);
// Cache keys may advance independently; the delivery contract is that campus loads the versioned root app.
assert.match(campus, /app\.jsx\?v=[^'"\s]+/);
