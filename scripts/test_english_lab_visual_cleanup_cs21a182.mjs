...
check(cleanup.includes('.elive-main-grid,.elive-join-grid,.aplay-shell'), 'cubre docente, estudiante y catálogo individual');
check(cleanup.includes("hideClosestCardByLabel(root, 'Banco pedagógico'"), 'oculta el diagnóstico interno del banco');
check(cleanup.includes("hideMessagePreview(root, audit)"), 'retira la vista previa redundante del mensaje');
check(cleanup.includes('Controlá la actividad y el avance del grupo.'), 'simplifica el encabezado docente');
check(cleanup.includes('Creá una sala, compartí el código y dirigí la actividad en vivo.'), 'simplifica la vista de creación');
...
console.log(JSON.stringify({ok:true, version:'CS21A182', checks:20}));
