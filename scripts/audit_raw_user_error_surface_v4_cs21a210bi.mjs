...
function isBareSetter(src,index){
...
function add(src,file,index,name,call){
  ...
  findings.push({file,line:lineOf(src,index),kind:custom?`${prop}_CUSTOM_SETTER`:`${prop}_SINK`,sink:name,text:call.replace(/\s+/g,' ').trim().slice(0,500)});
}
function scan(file){
  ...
  let m;
  while((m=re.exec(src))){
    if(!isBareSetter(src,m.index)) continue;
    const name=m[1];
...
console.log('CS21A210BI RAW USER ERROR SURFACE AUDIT V4');
...
console.log('NOTE=V4 preserves V3 semantics but only treats bare setter/toast/dispatch calls as React/UI sinks; member methods such as sessionStorage.setItem are excluded');
console.log('NOTE=Audit findings still require route/ownership review before any functional patch');
