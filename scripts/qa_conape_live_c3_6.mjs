...
const checks = [
  ...
  ['modal muestra código y etapa técnicos sanitizados', /function technical\(v, fallback\)/.test(row) && /\[\^A-Z0-9_\]/.test(row) && /Código: \$\{code\}/.test(row) && /Etapa: \$\{stage\}/.test(row)],
  ['modal no muestra message libre del bridge', /function submitFailure\(r\)/.test(row) && !/submitFailure[\s\S]{0,500}r\?\.message/.test(row) && /NO_DISPONIBLE/.test(row)],
  ['modal conserva saltos de línea técnicos', /\.vx-c33-banner\.err\{white-space:pre-line\}/.test(row)],
